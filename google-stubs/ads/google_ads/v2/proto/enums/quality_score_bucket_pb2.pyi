# @generated by generate_proto_mypy_stubs.py.  Do not edit!
import sys
from google.protobuf.descriptor import (
    Descriptor as google___protobuf___descriptor___Descriptor,
    EnumDescriptor as google___protobuf___descriptor___EnumDescriptor,
)

from google.protobuf.message import (
    Message as google___protobuf___message___Message,
)

from typing import (
    List as typing___List,
    Tuple as typing___Tuple,
    cast as typing___cast,
)


class QualityScoreBucketEnum(google___protobuf___message___Message):
    DESCRIPTOR: google___protobuf___descriptor___Descriptor = ...
    class QualityScoreBucket(int):
        DESCRIPTOR: google___protobuf___descriptor___EnumDescriptor = ...
        @classmethod
        def Name(cls, number: int) -> str: ...
        @classmethod
        def Value(cls, name: str) -> QualityScoreBucketEnum.QualityScoreBucket: ...
        @classmethod
        def keys(cls) -> typing___List[str]: ...
        @classmethod
        def values(cls) -> typing___List[QualityScoreBucketEnum.QualityScoreBucket]: ...
        @classmethod
        def items(cls) -> typing___List[typing___Tuple[str, QualityScoreBucketEnum.QualityScoreBucket]]: ...
        UNSPECIFIED = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 0)
        UNKNOWN = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 1)
        BELOW_AVERAGE = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 2)
        AVERAGE = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 3)
        ABOVE_AVERAGE = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 4)
    UNSPECIFIED = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 0)
    UNKNOWN = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 1)
    BELOW_AVERAGE = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 2)
    AVERAGE = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 3)
    ABOVE_AVERAGE = typing___cast(QualityScoreBucketEnum.QualityScoreBucket, 4)


    def __init__(self,
        ) -> None: ...
    @classmethod
    def FromString(cls, s: bytes) -> QualityScoreBucketEnum: ...
    def MergeFrom(self, other_msg: google___protobuf___message___Message) -> None: ...
    def CopyFrom(self, other_msg: google___protobuf___message___Message) -> None: ...
